...
sym : {X : Set}
   ...
   -> x == x'
   -> x' == x
sym {X} {x} {x'} p = eq-elim {Y = λ x x' _ -> x' == x}
                             p
                             (λ x -> refl x)
...
trans : {X : Set}
     ...
     -> x' == x''
     -> x == x''
trans {X} {x} {x'} {x''} p = eq-elim {Y = λ x x' _ -> x' == x'' -> x == x''}
                                     p
                                     (λ x q -> q)
...
transport : {X : Set}
         ...
         -> x == x'
         -> Y x -> Y x'
transport {X} {Y} p = eq-elim {Y = λ x x' _ -> Y x -> Y x'}
                              p
                              (λ x y -> y)
...
app-cong : {X Y : Set}
        ...
        -> x == x'
        -> f x == f x'
app-cong {X} {Y} {f} p = eq-elim {Y = λ x x' _ -> f x == f x'}
                                 p
                                 (λ x -> refl (f x))
...
dapp-cong : {X : Set}
         ...
         -> (p : x == x')
         -> transport p (f x) == f x'
dapp-cong {X} {Y} {f} p = eq-elim {Y = λ x x' p -> transport p (f x) == f x'}
                                  p
                                  (λ x -> refl (f x))
